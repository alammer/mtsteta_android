...
class MovieRepository() {
    ...
    suspend fun loadGenres(): List<String> = withContext(Dispatchers.IO) {
        ...
        if (localGenres.isNullOrEmpty()) {
            localGenres = fakeDataSource.genreList.also { genres ->
    ...
    fun loadMovieList(): List<MovieDto>? {
        ...
        if (localMovies.isNullOrEmpty()) {
            localMovies = fakeDataSource.getMovies()?.also { domainModelList ->
                    dataDao.insertAllMovies(domainModelList.map { it.toDataBaseModel() })
                }
        }
        return localMovies
    }
